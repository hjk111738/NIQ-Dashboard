...
def get_dashboard_data(
    ...
    try:
        ...
        """
        kpi_list = query_to_dict(conn, kpi_query)
        kpi = kpi_list[0] if kpi_list else {}
        
        total_sales = kpi.get("total_sales_eok", 0)
        lotte_sales = kpi.get("lotte_sales_eok", 0)
        ...
        """
        mfr_all = query_to_dict(conn, mfr_query)
        ...
        """
        brand_df = query_to_dict(conn, brand_query)
        ...
        """
        formatted_periods = [r[0] for r in conn.execute(period_meta_query).fetchall()]
        ...
        """
        table_df = query_to_dict(conn, table_query)

        return {
            "kpi": kpi, "has_dist": has_dist, "manufacturers": mfr_all[:8], "all_mfr_names": [m.get("manufacturer") for m in mfr_all],
